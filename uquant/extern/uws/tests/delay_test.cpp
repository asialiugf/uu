...
int main()
{
  ...
  h.onMessage([&t0,&t1](uWS::WebSocket<uWS::SERVER> *ws, char *message, size_t length, uWS::OpCode opCode) {
    char tmp[256];
    ...
    printf("server : thedifference is %ld\n",span1);
  ...
  // 客户端打印接收到的消息
  h.onMessage([&t0,&t1](uWS::WebSocket<uWS::CLIENT> *ws, char *message, size_t length, uWS::OpCode opCode) {
